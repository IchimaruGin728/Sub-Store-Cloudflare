...
工作流会自动检查并创建 Pages 项目。默认项目名是 `sub-store-cloudflare`，也可以在 GitHub Actions repository variables 中设置 `FRONTEND_PAGES_PROJECT` 覆盖。
...
| `CF_PAGES_DEPLOY_HOOK` | Cloudflare Pages Deploy Hook（可选；设置后前端走 Cloudflare Pages 构建额度） | `https://api.cloudflare.com/client/v4/pages/webhooks/deploy_hooks/...` |
可选 repository variable：

| Variable 名称 | 说明 | 默认值 |
|---------------|------|--------|
| `FRONTEND_PAGES_PROJECT` | Cloudflare Pages 项目名 | `sub-store-cloudflare` |

如果要使用 Cloudflare Pages paid 计划的构建额度，把 Pages 项目连接到这个仓库，并配置：

- Build command: `bash scripts/build-pages-frontend.sh`
- Build output directory: `frontend-dist/dist`
- GitHub Secret `CF_PAGES_DEPLOY_HOOK`: Pages 项目的 Deploy Hook URL
