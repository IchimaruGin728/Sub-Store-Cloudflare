...
> [!IMPORTANT]
> 项目方向已经调整为 **单用户、Cloudflare-native、独立实现**。当前保留官方 Sub-Store 后端同步作为过渡路径，前端/dashboard 代码已经移除，长期目标不是兼容官方前端或继续 patch 官方后端。详细路线见 [Cloudflare Native Rewrite](docs/CLOUDFLARE_NATIVE_REWRITE.md)。
...
- **脚本**：QuickJS 兼容实现
- **GeoIP**: 已实现，当前通过 Worker 设置/内部接口维护 mmdb URL
- **代理请求**: 不可用，但也不需要
...
> 
> Cloudflare build 会从官方仓库拉取当前记录的后端版本：
